...
char buf[ 256 ];
FILE *fp;
const char pid_path[ 256 ] = "/home/pi/Desktop/iRobotBluedoor/program/log/pid.txt";
const char tmp_log_path[ 256 ] = "/home/pi/Desktop/iRobotBluedoor/program/log.csv";

// --------------------- sensorInfo -------------------- ||
typedef struct
{
	int velocity;
	int Wheel_Overcurrents;
	int turning_radius;
	int distance;
	int angle;
...
int driveDistance (short vel, short rad, int dist, int interrupt)
{
	...
	if (drive (vel, rad) == -1 || (ret = waitDistance (dist, interrupt,4)) == INT_MIN || drive (0, 0) == -1)
		return INT_MIN;
 ...
 * 	\return		Distance travelled or INT_MIN on error
 */
int waitDistance (int dist, int interrupt,int Flag)
{
	int count = 0, current = 0;
	
	//reset sensor data
	if (INT_MIN == getDistance(Flag))
		return INT_MIN;

	while (1)
	{
		usleep (20000);
		current = getDistance(Flag);
...
* BRIEF : Wrappers of the each methods
* begin			startOI_MT
* end			stopOI_MT
* forward		drive
* backward		drive
* forwardDistance	driveDistance
* backwardDistance	driveDistance
* turnAngle		turn
* turn			turn
* wait			waitTime
|| -------------------------------------------------- */
...
|| -------------------------------------------------- */
int begin() {
	// -------------------- get monitor's PID from /monitor/pid.txt -------------------- ||
	/*fp = fopen( pid_path , "r" );
	if (fp == NULL) {
		perror( "fopen() pid_path:" );
		printf( "Please run monitor at first\n");
		return 0;
	}
	memset( &buf, 0, 256 );
	...
	// -------------------- monitor's PID is existed? -------------------- ||
	int r = kill( ( pid_t )pid, 0 );
	if ( r != 0 ) {
		// not exists
		printf("ERROR - the pid not exists\n");
		return -1;
	}
	else if( EPERM == errno ) {
		// not existing
	...
	// -------------------- open buf.txt -------------------- ||
	fp = fopen( tmp_log_path, "a" );
	if( fp == NULL ) {
		perror( "fopen() tmp_log_path:" );
		return -1;
	}
...
/* -------------------------------------------------- ||
	int stopOI_MT(const char* serial)
...
int end()
{
  funcLogger(0);
    ...
    sensor_info -> charge = getCharge();
    sensor_info -> voltage = readSensor(SENSOR_VOLTAGE);
    sensor_info -> current = readSensor(SENSOR_CURRENT);
    sensorLogger(4, sensor_info -> charge, 0, 0, 0);
...
|| -------------------------------------------------- ||*/
int drive (short vel, short rad)
{	int* sensors = (int*)malloc (36*sizeof(int));
	printf("drive()\n");
	sensors = getAllSensors();
	sensor_info -> velocity = getVelocity();
	sensor_info -> Wheel_Overcurrents = sensors[14-7];
	sensor_info -> turning_radius = getTurningRadius();
	sensor_info -> distance = getDistance(4);
	sensor_info -> angle = getAngle();
	sensor_info -> bumps_and_wheel_drops = getBumpsAndWheelDrops();
	sensor_info -> cliffs = getCliffs();
	sensor_info -> voltage = readSensor(SENSOR_VOLTAGE);
	sensor_info -> current = readSensor(SENSOR_CURRENT);
	sensor_info -> temperature = sensors[24-7];
	sensor_info -> charge = getCharge();
        sensor_info -> overcurrent = getOvercurrent();
	byte cmd[5];
...
* RETURN : Distance travelled or INT_MIN on error
|| -------------------------------------------------- */
int forwardDrive(short vel, int dist)
{
  //funcLogger(2, vel, dist);

	if (vel < 0 || dist < 0) return INT_MIN;
	...
	else
	{
		...
		if (drive (0, 0) == -1 ||drive (vel, rad) == -1 ||
		    (ret = waitDistance(dist, interrupt,0)) == INT_MIN)
		{
			ret = INT_MIN;
		}
		
		sensorLogger(5, sensor_info -> charge, dist, vel, 0, 1);
	}

	return ret;
}

int backwardDrive(short vel, int dist)
{
  //funcLogger(2, vel, dist);

	if (vel < 0 || dist < 0) return INT_MIN;

	vel = -vel; //diff from forward
	short rad = 0;
	int ret = 0;
	...
	{
		dist = -dist; //diff from forward
		int interrupt = 1;

		if (drive(0, 0) == -1 || drive (vel, rad) == -1 ||
		    (ret = waitDistance(dist, interrupt,1)) == INT_MIN)
		{
			ret = INT_MIN;
		}

		sensor_info -> velocity *= -1;
		sensor_info -> distance *= -1;
		dist = dist*-1;
		vel = vel*-1;
		sensorLogger(5, sensor_info -> charge, dist, vel, 0, 2);
	}
...
* RETURN : Angle turned or INT_MIN on error
|| -------------------------------------------------- */
int forwardTurn(short vel, short rad, int angle)
{
  //funcLogger(3, vel, rad, angle);

	if (vel < 0 || angle < 0) return INT_MIN;

	int ret = 0;
	int t_dist;

	if(rad ==0 || rad == 1)
	  t_dist = 235*3*angle/360;
	else{
	  t_dist = 2*rad*3*angle/360;
	  //printf("%d", t_dist);
	}
	...
	else
	{
		...
		if(angle < 0)
		  angle = angle*-1;
		sensorLogger(5, sensor_info -> charge, t_dist, vel, angle, 3);
	}

	return ret;
}

int backwardTurn(short vel, short rad, int angle)
{
  //funcLogger(3, vel, rad, angle);

	if (vel < 0 || angle < 0) return INT_MIN;

	vel = -vel; //diff from forwardTurn
	int ret = 0;
	int t_dist;
	...
	else
	{
		...
		sensor_info -> velocity *= -1;
		sensor_info -> turning_radius *= -1;
//		sensor_info -> angle *= -1;
		vel = vel*-1;
		if(angle < 0)
		  angle = angle*-1;
		sensorLogger(5, sensor_info -> charge, t_dist, vel, angle,4);
	}
...
* BRIEF : Loggers
* sensorLogger		Original (different point from functionLogger is just initialization)
* functionLogger	Original(define funcLogger on header file)
|| -------------------------------------------------- */
...
* 4	turning radius
* 5	over current
* 6	bumps and wheel drops
* 7	cliffs
|| -------------------------------------------------- */
void sensorLogger(const int argNum, ...)
{
/*
	printf(" = = = = = sensorLogger() = = = = = \n");
	printf("velocity = %d\n", sensor_info -> velocity);
	printf("turning_radius = %d\n", sensor_info -> turning_radius);
	printf("distance = %d\n", sensor_info -> distance);
	printf("angle = %d\n", sensor_info -> angle);
	printf("bumps_and_wheel_drops = %d\n", sensor_info -> bumps_and_wheel_drops);
	printf("cliffs = %d\n", sensor_info -> cliffs);
	printf("charge = %d\n", sensor_info -> charge);
	printf("overcurrent = %d\n", sensor_info -> overcurrent);
*/
	...
	fprintf( fp, "sensor,%06ld", tv.tv_sec * 1000000 + tv.tv_usec );
	tm = localtime( &tv.tv_sec );
	fprintf( fp, ",%d,%02d,%02d,%02d,%02d,%02d", 1900 + tm->tm_year, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec );
	//Windows
	#if defined(WIN32) || (!defined(__GNUC__) && !defined(__clang__))
		fprintf( fp, ",%06ld", tv.tv_usec );
	//Linux
...
* BRIEF : Output log about executed functions
* PARAM : funcName ... function name
* PARAM : argNum ... argument number
|| -------------------------------------------------- */
void functionLogger(const char* funcName, const int argNum, ...)
{
	struct timeval tv;
	struct tm *tm;
	...
	fprintf( fp, "function,%06ld", tv.tv_sec * 1000000 + tv.tv_usec );
	tm = localtime( &tv.tv_sec );
	fprintf( fp, ",%d,%02d,%02d,%02d,%02d,%02d", 1900 + tm->tm_year, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec );
	//Windows
	#if defined(WIN32) || (!defined(__GNUC__) && !defined(__clang__))
	...
	sensor_info = (sensor_info_t*) malloc(sizeof(sensor_info_t));

	sensor_info -> velocity = 0;
	sensor_info -> Wheel_Overcurrents = 0;
	sensor_info -> turning_radius = 0;
	sensor_info -> distance = 0;
	sensor_info -> angle = 0;
	sensor_info -> bumps_and_wheel_drops = 0;
	sensor_info -> cliffs = 0;
	sensor_info -> voltage = 0;
	sensor_info -> temperature = 0;
	sensor_info -> current = 0;
	sensor_info -> charge = 0;
	sensor_info -> overcurrent = 0;
